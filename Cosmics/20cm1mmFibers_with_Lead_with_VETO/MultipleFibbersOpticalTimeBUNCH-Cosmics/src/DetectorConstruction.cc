...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
    ...
    G4double worldExt=50*cm;
    //G4double worldExt=150*cm;//use 150cm as the Cry plan was choosed to have 3m side //REAL
    G4double worldHeight=40*cm;//REAL

    //External lead size
    G4double halflengthExtLeadX = 30*cm;
    G4double halflengthExtLeadY = 30*cm;
    //G4double halflengthExtLeadX = 74*cm;//REAL
    //G4double halflengthExtLeadY = 30*cm;//REAL
    ...
    //Veto
    G4double halfVetoX = 8.5*cm;
    G4double halfVetoY = 22.5*cm;
    G4double halfVetoZ = 0.5*cm;
    ...
    //Make Veto
    G4VSolid* vetoBoxSolid = new G4Box("vetoBoxSolid", halfVetoX, halfVetoY, halfVetoZ);
    G4LogicalVolume* vetoBoxLogical = new G4LogicalVolume(vetoBoxSolid, polystyrene, "vetoBoxLogical");
    ...
    //VetoPhysical
    G4double zTranslationVeto=155*mm;
    G4ThreeVector positionVetoHigh1 = G4ThreeVector(0., 0., zTranslationVeto);
    G4ThreeVector positionVetoDown1 = G4ThreeVector(0., 0., -zTranslationVeto);

    G4Transform3D transformVetoHigh1 = G4Transform3D(rotm,positionVetoHigh1);
    G4Transform3D transformVetoDown1 = G4Transform3D(rotm,positionVetoDown1);
    ...
    G4OpticalSurface* opSilicon_Glass = new G4OpticalSurface("SiliconGlassSurface");
        ...
        opSilicon_Glass->SetFinish(polishedteflonair);                 // SetFinish
    ...
    G4VisAttributes* visAttributesLead = new G4VisAttributes(G4Colour(0.6,0.6,0.6));
    visAttributesLead->SetVisibility(true);
    leadBoxLogical->SetVisAttributes(visAttributesLead);
    fVisAttributes.push_back(visAttributesLead);
    ...
    //visAttributes = new G4VisAttributes(G4Colour(0.0,1.0,0x.0));
    //visAttributes->SetVisibility(true);
    ...
    G4VisAttributes* visAttributesVeto = new G4VisAttributes(G4Colour(1.0,1.0,0.0));
    visAttributesVeto->SetVisibility(true);
    vetoBoxLogical->SetVisAttributes(visAttributesVeto);
    fVisAttributes.push_back(visAttributesVeto);
...
void DetectorConstruction::ConstructSDandField()
{
    ...
    vetoSD* aVetoSD = new vetoSD(vetoSDname,"vetoHitsCollection");
    G4SDManager::GetSDMpointer()->AddNewDetector(aVetoSD);
    //// Setting aVetoSD to all logical volumes with the same name
    //// of "vetoLogical".

    SetSensitiveDetector("vetoBoxLogical", aVetoSD, true);
