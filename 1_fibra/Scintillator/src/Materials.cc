...
void Materials::CreateMaterials()
{
  ...
  fSilice = fNistMan->FindOrBuildMaterial("G4_Si");
...
//
 ...
 const G4int nEntriesAbsorptionWater = sizeof(photonAbsorptionWater)/sizeof(G4double);
  ...
  mptWater->AddProperty("RINDEX",       photonEnergyWater, refractiveIndexWater,nEntriesWater)
        ->SetSpline(true);
  mptWater->AddProperty("ABSLENGTH",    photonAbsorptionWater, absorptionWater,     nEntriesAbsorptionWater)
        ->SetSpline(true);
...
//  assert(sizeof(absGlass) == sizeof(photonEnergyBorosilicate));
 ...
 mptPhoto = new G4MaterialPropertiesTable();
  ...
  fSilice->SetMaterialPropertiesTable(mptPhoto);
