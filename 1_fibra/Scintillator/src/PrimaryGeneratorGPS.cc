...
void PrimaryGeneratorGPS::GeneratePrimaries(G4Event* anEvent)
{
  ...
  // G4double sourceRadius=0.55*mm;
   posDist->SetRadius(sourceRadius);


   //G4cout<<"RADIUS "<<sourceRadius<<G4endl;


   G4double sourceHalfLength=fDetConstruction->GetSourceLength();
   //G4double sourceHalfLength=10*cm;
      //G4cout<<"Length "<<sourceHalfLength/cm<<G4endl;
...
//const G4int nFibbers=fDetConstruction->GetNFibbers();
  ...
  // G4int fibberNumber=std::abs(G4UniformRand()*(nFibbers));
   ...
   //G4String confinedToSolid="sourcePhysical_"+std::to_string(fibberNumber);
    G4String confinedToSolid="sourcephysical";
...
*/
  ...
  //analysisManager->FillNtupleDColumn(0,5, fSourceT0);
  //analysisManager->FillNtupleIColumn(0,6, fibberNumber);
  //analysisManager->FillNtupleDColumn(0,7, distanceToSurface);
